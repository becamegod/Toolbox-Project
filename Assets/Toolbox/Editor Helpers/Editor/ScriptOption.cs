...
public class ScriptOptions
{
    ...
    static void AutoRefreshToggle()
    {
        ...
        if (status == 1) EditorPrefs.SetInt("kAutoRefresh", 0);
        else EditorPrefs.SetInt("kAutoRefresh", 1);
    }
    ...
    {
        var status = EditorPrefs.GetInt("kAutoRefresh");
        if (status == 1) Menu.SetChecked("Tools/Auto Refresh", true);
        else Menu.SetChecked("Tools/Auto Refresh", false);
        return true;
    }
